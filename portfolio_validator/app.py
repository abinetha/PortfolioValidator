...
def index():
    ...
    if request.method == 'POST':
        ...
        if not html_file or not css_file:
            errors = ["Please upload both HTML and CSS files."]
        else:
            # Check if the uploaded files are of the correct type
            if html_file.filename.split('.')[-1].lower() != 'html':
                errors = ["Please upload an HTML file."]
            elif css_file.filename.split('.')[-1].lower() != 'css':
                errors = ["Please upload a CSS file."]
            else:
                # Read HTML and CSS file contents
                html_content = html_file.read().decode('utf-8')
                css_content = css_file.read().decode('utf-8')
                ...
                if not errors:
                    message = "Congratulations! Your portfolio meets all the criteria."

    return render_template('index.html', errors=errors, message=message)
